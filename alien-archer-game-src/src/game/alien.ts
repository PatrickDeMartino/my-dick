...
export function createAlien(kind: AlienId): AlienRig {
  ...
  function makeArm(side: number) {
    ...
    chest.add(arm);
    add(arm, new THREE.SphereGeometry(0.07, 10, 8), skinDark, 0, 0, 0);
    add(arm, new THREE.CylinderGeometry(0.045, 0.055, 0.34, 10), skin, 0, -0.17, 0);
    const forearmG = new THREE.Group();
    forearmG.position.y = -0.34;
    arm.add(forearmG);
    add(forearmG, new THREE.SphereGeometry(0.05, 8, 6), skinDark, 0, 0, 0);
    add(forearmG, new THREE.CylinderGeometry(0.035, 0.045, 0.32, 10), skin, 0, -0.16, 0);
    const hand = new THREE.Group();
    hand.position.y = -0.34;
    forearmG.add(hand);
    add(hand, new THREE.SphereGeometry(0.05, 8, 6), skin, 0, 0, 0);
    for (let i = 0; i < 3; i++) {
      const f = new THREE.ConeGeometry(0.014, 0.07, 5);
      add(hand, f, skinDeep, (i - 1) * 0.028, -0.05, 0.02, 1.1);
    }
  ...
  // The active weapon is anchored at the rounded end of the right arm. The
  // weapon origin sits at the pistol grip; +Z is always the barrel direction.
  const gunMount = new THREE.Group();
  gunMount.position.set(0, 0.02, 0.03);
...
export function updateAlien(rig: AlienRig, a: AlienAnim) {
  ...
  // One-handed weapon pose: the right shoulder follows the sight line while
  // the free arm hangs and swings naturally at the alien's side.
  rig.armL.rotation.set(0.08 + swing * 0.34, 0.04, 0.14);
  rig.forearmL.rotation.set(-0.08 - Math.max(0, swing) * 0.16, 0, 0);
  rig.armR.rotation.set(-1.48 - kick * 0.18, -0.04, -0.08);
  rig.forearmR.rotation.set(-0.08 - rel * 0.42, 0, 0);
  ...
  gun.root.rotation.set(0, 0, 0);
  gun.root.position.set(0, 0.055 - kick * 0.035, ak ? 0.08 : 0.045 - kick * 0.06);
  ...
  if (ak) {
    rig.gunMount.position.set(0, 0.02 - rel * 0.05, 0.03);
    if (gun.mag) gun.mag.visible = rel < 0.45 || rel > 0.7;
  } else {
    rig.gunMount.position.set(0, 0.02, 0.03);
    if (gun.cylinder) {
