...
test("Penguin Town V2 keeps the mobile-first 3D island and circular popup system", async () => {
  ...
  const banner = await readFile(new URL("../app/components/SiteBanner.tsx", import.meta.url), "utf8");
  const townData = await readFile(new URL("../app/urf/townData.ts", import.meta.url), "utf8");
  const can = await readFile(new URL("../app/components/Can3D.tsx", import.meta.url), "utf8");

  assert.match(town, /<BuildingPopup/);
  assert.match(town, /className="town-frame"/);
  assert.match(town, /lockedGlow/);
  assert.match(town, /dart-marker/);
  assert.match(town, /frontness > \.82/);
  assert.match(town, /--dart-launch-x/);
  assert.match(town, /penguin-town-camera/);
  assert.match(town, /buildingYaw/);
  assert.match(scene, /popupPullback/);
  assert.match(scene, /buildCliffFacetBand/);
  assert.match(scene, /mode = "ragdoll"/);
  assert.match(scene, /selectionRing/);
  assert.match(scene, /controls\.minDistance = compactMode \? 7\.2 : 5\.2/);
  assert.match(scene, /const courses = \[/);
  assert.match(scene, /isBuildingLabel/);
  assert.match(scene, /Math\.PI \/ 2/);
  assert.match(scene, /penguin-town-camera/);
  assert.match(scene, /buildingYaw/);
  assert.match(popup, /building-popup-circle/);
  assert.match(popup, /JellyButtons shape="circle"/);
  assert.match(popup, /building-popup-rotation-ring/);
  ...
  assert.match(styles, /town-camera-controls/);
  assert.match(styles, /@keyframes dart-flight/);
  assert.match(styles, /scale\(5\.2\)/);
  assert.match(banner, /<Can3D size=\{44\}/);
  assert.match(banner, /<OilGauge \/>/);
  assert.match(banner, /variant="yoohoo"/);
  assert.match(townData, /ALIEN PERVY PEEPER/);
  assert.match(townData, /visualScale: 2\.45/);
  assert.match(townData, /rat-photo-hires\.jpg/);
  assert.match(townData, /PINGUWINOORU/);
  assert.match(townData, /Make great very good number one glorious Leader of supper fast number 1 sweatshop\./);
  assert.match(townData, /VICHEAL MIC/);
  assert.match(townData, /DOG FIGHT ENTHUSIAST/);
  assert.match(can, /shoulderGeo/);
