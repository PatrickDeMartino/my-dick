import { c as e, l as t } from "./three.module-h2PuqYDi.js";
//#region alien-archer-game-src/node_modules/three/examples/jsm/utils/BufferGeometryUtils.js
function n(e, n = !1) {
	let i = e[0].index !== null, a = new Set(Object.keys(e[0].attributes)), o = new Set(Object.keys(e[0].morphAttributes)), s = {}, c = {}, l = e[0].morphTargetsRelative, u = new t(), d = 0;
	for (let t = 0; t < e.length; ++t) {
		let r = e[t], f = 0;
		if (i !== (r.index !== null)) return console.error("THREE.BufferGeometryUtils: .mergeGeometries() failed with geometry at index " + t + ". All geometries must have compatible attributes; make sure index attribute exists among all geometries, or in none of them."), null;
		for (let e in r.attributes) {
			if (!a.has(e)) return console.error("THREE.BufferGeometryUtils: .mergeGeometries() failed with geometry at index " + t + ". All geometries must have compatible attributes; make sure \"" + e + "\" attribute exists among all geometries, or in none of them."), null;
			s[e] === void 0 && (s[e] = []), s[e].push(r.attributes[e]), f++;
		}
		if (f !== a.size) return console.error("THREE.BufferGeometryUtils: .mergeGeometries() failed with geometry at index " + t + ". Make sure all geometries have the same number of attributes."), null;
		if (l !== r.morphTargetsRelative) return console.error("THREE.BufferGeometryUtils: .mergeGeometries() failed with geometry at index " + t + ". .morphTargetsRelative must be consistent throughout all geometries."), null;
		for (let e in r.morphAttributes) {
			if (!o.has(e)) return console.error("THREE.BufferGeometryUtils: .mergeGeometries() failed with geometry at index " + t + ".  .morphAttributes must be consistent throughout all geometries."), null;
			c[e] === void 0 && (c[e] = []), c[e].push(r.morphAttributes[e]);
		}
		if (n) {
			let e;
			if (i) e = r.index.count;
			else if (r.attributes.position !== void 0) e = r.attributes.position.count;
