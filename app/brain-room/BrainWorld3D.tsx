...
type Controls={subject:BrainSubject;walkMode:boolean;ragdoll:boolean;reset:number;spawn:'room'|'lab'|'field';paused:boolean;onInteract:()=>void;onNearComputer:(near:boolean)=>void};
export default function BrainWorld3D(props:Controls) {
  ...
  useEffect(()=>{
    ...
    const player={p:settings.current.spawn==='lab'?LAB_ENTRY.clone():settings.current.spawn==='field'?new T.Vector3(-21,.4,-13):SPAWN.clone(),v:new T.Vector3(),grounded:false};const radius=.3;
    ...
    const spawnItem=(e:Event)=>{const event=e as CustomEvent<{kind:string}>;event.preventDefault();if(['baboon','gorilla','chimpanzee','yeti'].includes(event.detail.kind))playground.spawnSubject(event.detail.kind as 'baboon');else playground.spawn(event.detail.kind);};
    ...
    const action=(e:Event)=>{const a=(e as CustomEvent).detail;if(a==='pickup')playground.pickup();if(a==='throw')playground.throw(19);if(a==='hit'){ray.setFromCamera(new T.Vector2(),camera);playground.punch(ray);}};
    window.addEventListener('trip-world-spawn',spawnItem);window.addEventListener('trip-world-clear',clearItems);window.addEventListener('trip-world-settings',worldSettings);window.dispatchEvent(new Event('trip-world-ready'));window.addEventListener('brain-repair',repair);window.addEventListener('brain-action',action);
    ...
    const down=(e:PointerEvent)=>{if(settings.current.paused)return;renderer.domElement.setPointerCapture(e.pointerId);lastX=e.clientX;lastY=e.clientY;clickAt=performance.now();clickDistance=0;locate(e);if(playground.held){dragging=true;return;}
    ...
    const up=()=>{if(!settings.current.paused){if(playground.held)playground.throw(14+Math.min(1.5,(performance.now()-clickAt)/1000)*9);else if(!held&&clickDistance<7)playground.punch(ray);}dragging=false;held?.release();held=null;grabPlane=null;};const visibility=()=>{active=!document.hidden;blur();};
    ...
    const animate=()=>{
      ...
      if(state.reset!==reset||state.spawn!==spawn){reset=state.reset;spawn=state.spawn;updateCreature(state.subject,spawn==='lab'?LAB_ENTRY.clone():spawn==='field'?new T.Vector3(-21,.4,-13):SPAWN.clone());yaw=spawn==='lab'?0:spawn==='field'?-.9:.5;pitch=.22;}
    ...
    return()=>{window.removeEventListener('trip-world-spawn',spawnItem);window.removeEventListener('trip-world-clear',clearItems);window.removeEventListener('trip-world-settings',worldSettings);window.removeEventListener('brain-repair',repair);window.removeEventListener('brain-action',action);level.dreamscape.dispose();cancelAnimationFrame(frame);observer.disconnect();window.removeEventListener('brain-stick',stickInput);window.removeEventListener('trip-bongo-action',bongoAction);window.removeEventListener('keydown',keydown);window.removeEventListener('keyup',keyup);window.removeEventListener('blur',blur);window.removeEventListener('brain-input',touchInput);document.removeEventListener('visibilitychange',visibility);canvas.removeEventListener('pointerdown',down);canvas.removeEventListener('pointermove',move);canvas.removeEventListener('pointerup',up);canvas.removeEventListener('pointercancel',blur);disposeObject(scene);colliderMeshes.forEach(disposeObject);renderer.dispose();canvas.remove();};
  },[]);
