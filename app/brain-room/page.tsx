...
export default function BrainRoom(){
 const [subject,setSubject]=useState<BrainSubject>('pongo'),[pov,setPov]=useState(false),[ragdoll,setRagdoll]=useState(false),[reset,setReset]=useState(0),[spawn,setSpawn]=useState<'room'|'lab'|'field'>('room');
 ...
 useEffect(()=>{const arrival=new URLSearchParams(window.location.search).get('arrival');if(arrival==='lab'||arrival==='field')setSpawn(arrival);},[]);
