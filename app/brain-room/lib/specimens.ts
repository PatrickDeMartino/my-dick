...
export function makeSpecimen(kind:SpecimenKind,index:number){
 ...
 const body=new T.Group();root.add(body);part(body,fur,[0,1.58,-.08],[config.w,config.h,.37]);part(body,skin,[0,1.6,.266],[config.w*.66,config.h*.58,.055]);
 const head=new T.Group();head.position.set(0,2.26+(kind==='yeti'?.2:0),0);root.add(head);
 ...
 for(const side of [-1,1]){
   ...
   part(root,fur,[side*(config.w+.1),1.68,.03],[.19,.35,.19]);part(root,fur,[side*.68,1.4,.35],[.145,.14,.46]);part(root,skin,[side*.68,1.39,.72],[.155,.09,.2]);
   for(let i=0;i<4;i++)part(root,skin,[side*.68+(i-1.5)*.063,1.37,.87],[.026,.03,.1]);
   part(root,fur,[side*.31,.93,.43],[.24,.22,.47]);part(root,fur,[side*.36,.56,.74],[.145,.35,.15]);part(root,skin,[side*.36,.26,.91],[.21,.105,.32]);
   for(let i=0;i<3;i++)part(root,skin,[side*.36+(i-1)*.12,.25,1.16],[.065,.068,.115]);
 ...
 root.userData={head,body};batchParts(root);return root;
}
