...
export default function AssetsRoomPage(){
 ...
 return <main className="asset-room"><AssetRoom3D onCount={onCount} onActive={onActive} onSelected={onSelected}/><a className="asset-room__back" href="/brain-room">← BRAIN ROOM</a>
  ...
  {open&&<aside className="asset-room__menu" aria-label="Site asset catalog"><button className="menu-close" type="button" onClick={()=>setOpen(false)} aria-label="Collapse asset catalog">×</button><header><b>SPAWN LIBRARY</b><small>{SITE_ASSETS.length} SHARED ASSETS</small></header>
   ...
   <section className="asset-room__inspector"><span>LIVE ENTITY</span><b>{entity?.name??"CLICK A MODEL"}</b><div className="asset-room__modes">{(["display","ragdoll","function"] as AssetMode[]).map(m=><button type="button" key={m} disabled={!entity} className={entity?.mode===m?"is-active":""} onClick={()=>setMode(m)}>{m.toUpperCase()}</button>)}</div><label>SIZE <output>{scale.toFixed(2)}×</output><input type="range" min="0.25" max="3" step="0.05" value={scale} disabled={!entity} onChange={e=>setEntityScale(Number(e.target.value))}/></label></section>
