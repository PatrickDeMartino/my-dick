...
function readStored(key: string, fallback: number) {
  try {
    const value = Number.parseInt(window.localStorage.getItem(key) ?? String(fallback), 10);
...
export default function SiteBanner() {
  ...
  const isBongo = pathname?.startsWith("/bongo") ?? false;
  ...
  useEffect(() => {
    ...
    const syncFrame = window.requestAnimationFrame(() => {
      setAmount(readStored(STORAGE_KEY, 69));
      setBananas(readStored(BANANA_STORAGE_KEY, 69));
      setYoohoo(readStored(YOOHOO_STORAGE_KEY, 69));
    });
    ...
    const onReward = (event: MessageEvent) => {
      ...
      const next = readStored(STORAGE_KEY, 69) + Math.floor(reward);
      try {
    ...
    const onStorage = (event: StorageEvent) => {
      if (event.key === STORAGE_KEY) setAmount(readStored(STORAGE_KEY, 69));
      if (event.key === BANANA_STORAGE_KEY) setBananas(readStored(BANANA_STORAGE_KEY, 69));
      if (event.key === YOOHOO_STORAGE_KEY) setYoohoo(readStored(YOOHOO_STORAGE_KEY, 69));
    };
