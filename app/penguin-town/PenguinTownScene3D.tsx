"use client";
import {seededRandom,buildNoiseBumpTexture,box,cylinder,sphere,beamBetween,makeLabel,makeStripeTexture,buildIceBrickTexture,buildPlankTexture,buildBunting,buildPlane,buildRivetTexture,buildTelescope,buildCircus,buildIgloo,buildSweatshop,buildBoat,buildArena,buildBuildingModel,buildPenguin} from '../world/models/town';
...
export default function PenguinTownScene3D({
  ...
  useEffect(() => {
    ...
    const universe=new WorldSimulation(scene,camera,renderer.domElement,{ground:.4,bounds:80,spawnPoint:()=>controls.target.clone().add(new THREE.Vector3(0,3,0)),onControl:e=>controls.enabled=!e});
    ...
    function step() {
      ...
      for (const penguin of penguins) {
        ...
        if (penguin.mode === "selected") {
          ...
          penguin.velocity.y -= gravityAcceleration() * frameDeltaSec;
      ...
      controls.update();
      universe.update(frameDeltaSec);universe.followCamera();renderer.render(scene, camera);
    }
    raf = requestAnimationFrame(step);
    ...
    return () => {
      ...
      universe.dispose();controls.dispose();
