...
import { makeSandboxProp, type SpawnKind } from "./sandbox";
...
export class Game {
  ...
  private spawned: { root: THREE.Group; velocity: THREE.Vector3; spin: THREE.Vector3 }[] = [];
  ...
  constructor(private canvas: HTMLCanvasElement) {
    ...
    this.world.globe.group.visible = false;
    this.world.spores.visible = false;
    ...
    this.onSpawnMessage = (event) => {
      ...
      const allowed: SpawnKind[] = ["vehicle","jetpack","ak47","revolver","bow","arrow","pepsi","yoohoo","biplane","penguin","bongo"];
      if (allowed.includes(kind as SpawnKind)) this.spawnProp(kind as SpawnKind);
    };
    this.onVis = () => {
  ...
  private fixed(dt: number) {
    ...
    if (wishLen > 0.05) {
      ...
      this.facingYaw = lerpAngle(this.facingYaw, this.camYaw, 1 - Math.exp(-10 * dt));
  ...
  private spawnProp(kind: SpawnKind) {
    const root = makeSandboxProp(kind);
    _fwd.set(-Math.sin(this.camYaw), 0, -Math.cos(this.camYaw));
    root.position.set(this.pos.x + _fwd.x * 3, this.pos.y + 3.6, this.pos.z + _fwd.z * 3);
    root.rotation.y = this.camYaw;
    this.scene.add(root);
    this.spawned.push({
      root,
      velocity: new THREE.Vector3(_fwd.x * (kind === "biplane" ? 8 : 2.5), kind === "jetpack" ? 4 : 1.2, _fwd.z * (kind === "biplane" ? 8 : 2.5)),
      spin: new THREE.Vector3(Math.random() * 2 - 1, Math.random() * 2 - 1, Math.random() * 2 - 1),
    });
    if (this.spawned.length > 40) {
  ...
  private stepSpawned(dt: number) {
    for (const item of this.spawned) {
      ...
      item.root.traverse((node) => { if (node.userData.propeller) node.rotation.z += dt * 28; });
      const ground = sampleGround(item.root.position.x, item.root.position.z);
      if (ground.island && item.root.position.y < ground.y + .35) {
        item.root.position.y = ground.y + .35;
        item.velocity.y = Math.abs(item.velocity.y) * .42;
        item.velocity.x *= .83;
        item.velocity.z *= .83;
        item.spin.multiplyScalar(.9);
      }
    }
  }
  ...
  dispose() {
    ...
    window.removeEventListener("message", this.onSpawnMessage);
