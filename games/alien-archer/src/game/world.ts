...
export const ISLANDS: Island[] = [
  { x: 0, z: 0, radius: 48, topY: 0 },
];

export function terrainHeight(x: number, z: number) {
  return Math.sin(x * .13) * .58 + Math.cos(z * .11) * .42 + Math.sin((x + z) * .055) * .72;
}
...
export function sampleGround(x: number, z: number): { y: number; island: Island | null } {
  ...
  for (const isl of ISLANDS) {
    ...
    if (d < isl.radius * 0.92) {
      ...
      const y = isl.topY + terrainHeight(x, z) * Math.min(1, edge * 4);
      if (y >= bestY) {
        bestY = y;
        best = isl;
      }
...
function islandGeometry(isl: Island, rng: () => number) {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    const j = (rng() - 0.5) * 0.16 * d;
    pos.setY(i, terrainHeight(isl.x + x, isl.z + z) + j);
  }
