...
const electron=path.join(root,'App/electron.exe'),launcher=path.join(root,'App/Triptotropic.exe');if(fs.existsSync(electron)){if(!fs.existsSync(launcher))fs.renameSync(electron,launcher);else if(fs.readFileSync(electron).equals(fs.readFileSync(launcher)))fs.unlinkSync(electron);else{fs.copyFileSync(electron,launcher);fs.unlinkSync(electron);}}
